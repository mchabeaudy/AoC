...
public class Day19B {
  ...
  public static void main(String[] args) {
    try (var linesStream = Files.lines(Paths.get(getSystemResource("day19.txt").getPath()))) {
      ...
      long max = scanners.stream()
          .mapToLong(scanner -> scanners.stream()
              .mapToLong(s -> s.getDist().manhattanDist(scanner.getDist())).max().orElse(0))
          .max()
          .orElse(0);
  ...
  static class Scanner {
    ...
    public Point getDist() {
      return dist;
    }

    public void setDist(Point dist) {
      this.dist = dist;
    }
  }
