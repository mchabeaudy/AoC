...
public class Day18B {
    ...
    try (var linesStream = Files.lines(
      ...
      for (int i = 0; i < lines.size() - 1; i++) {
        ...
        for (int k = i; k < lines.size(); k++) {
          var addition = "[" + line + "," + lines.get(k) + "]";
          var mag = getMag(reduceNumber(addition)).replace("[", "").replace("]", "");
          max = Math.max(max, parseLong(mag));
        }
      }
  ...
  private static String getMag(String number) {
